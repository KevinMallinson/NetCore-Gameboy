...
namespace Hardware.Processor
{
    ...
    {
        public Func<ushort, IInstruction> Get(int opcode) => _dictionary[opcode];
        private readonly Dictionary<int, Func<ushort, IInstruction>> _dictionary;

        public InstructionTable(IRegisterIO registerIO)
        {
            var registers = registerIO;
            _dictionary = new Dictionary<int, Func<ushort, IInstruction>>
            {
                // --------------------------- //
